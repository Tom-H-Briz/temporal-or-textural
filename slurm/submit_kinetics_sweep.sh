#!/bin/bash
# Submits the K400 dim_mean sweep, then both K400 SAE training sweeps (x8k64 and
# x16k128) chained on it via SLURM's job-array dependency — one command instead of
# waiting and resubmitting by hand.
#
...
# rather than all training tasks waiting for the slowest dim_mean layer. Both configs
# depend on the same dim_mean job — expansion/k don't affect dim_mean.
...
X8_JOBID=$(sbatch --parsable --dependency=aftercorr:$DIM_MEAN_JOBID train_sae_vm_kinetics_l5_l7_l9.sh)
echo "Submitted x8k64 training sweep: $X8_JOBID (each layer waits on dim_mean job $DIM_MEAN_JOBID, same layer)"

X16_JOBID=$(sbatch --parsable --dependency=aftercorr:$DIM_MEAN_JOBID train_sae_vm_kinetics_x16k128_l5_l7_l9.sh)
