#!/bin/bash
#SBATCH --job-name=tot_train_sae_vm_k400_l3
#SBATCH --output=train_sae_vm_k400_l3_%A_%a.out
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --time=06:00:00
#SBATCH --array=3

# L3 extension of train_sae_vm_kinetics_l5_l7_l9.sh — same 6h ceiling reasoning
# (K400 clips are full ~10s YouTube videos, decode-per-clip dominates over layer
# choice, so no time-budget change for L3 vs L5/L7/L9).
#
# Prereq: compute_dim_mean_vm_kinetics_l3.sh completed — writes
# vmae_kinetics400_layer3_dim_mean.pt, using the same VIDEO_DIR override as below.
