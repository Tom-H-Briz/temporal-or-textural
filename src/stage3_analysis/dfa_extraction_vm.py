"""
Comprehensive VM DFA attribution pass — 32 SL classes, layer 7.

One parquet: dfa_vm_k64_l7_sl32_{date}.parquet
Per-clip rows: metadata + scalars + sparse top-200 DFA features.
Full 6144-dim vectors are NOT persisted.

Usage:
    uv run python src/stage3_analysis/dfa_extraction_vm.py
"""
...
def _resolve_cfg(cfg: dict) -> dict:
    ...
    return {
        ...
        "sae_k":         64,
        "out_name":      f"dfa_vm_k64_l{cfg['layer']}_sl32_{date}",
    }
...
def main() -> None:
    ...
    cfg      = {**CFG, **resolved}
    log.info(f"Layer {cfg['layer']}  checkpoint={Path(cfg['sae_path']).name}  sae_k={cfg['sae_k']}")
